...
const d = (d3.json("conversations_worst_turns.json")).then(function(da){
    console.log(da)
...
function generateClipBar(conversation, clips) {
  ...
  header.innerHTML = "Listen to speakers:";
...
function generateTurnsBar(conversation, clips) {
  //clips = dict of clipsecond:error
  const bar = document.getElementById('turnClips');
  ...
  header.innerHTML = "Listen to worst turns:";
  bar.appendChild(header);
  for(turn in clips){
    clipButton = document.createElement('a');
     clipButton.innerHTML = clips[turn].toFixed(2);
    clipButton.className = 'big-button'
   
    clipButton.setAttribute('href',"https://app.lvn.org/conversation/" +conversation+"?t="+Math.round(turn).toString());
    bar.appendChild(clipButton);
  }
  return bar;
}
...
function addSpeakerGraph(scatter_data) {
 const svg = d3.select('#speakerGraph')
  ...
  return svg.node();
    ...
    generateTurnsBar(conversation, wer_data.worst_turns);
    addWERGraph();
